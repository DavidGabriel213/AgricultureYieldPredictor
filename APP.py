...
app=Flask(__name__)
processor_cat=joblib.load('processor_cat.joblib')
processor_reg=joblib.load('processor_reg.joblib')
...
def myfunc():
    ...
    if request.method=="POST":
        ...
        # features engineered
        Rainfall=np.log1p(Rainfall)
        SoilPH=np.log1p(SoilPH)
        Fertilizer=np.log1p(Fertilizer)
        Pesticide=np.log1p(Pesticide)
        DistanceToMarket=np.log1p(DistanceToMarket)
        PlantingDensity=np.log1p(PlantingDensity)
        FarmSize=np.log1p(FarmSize)
